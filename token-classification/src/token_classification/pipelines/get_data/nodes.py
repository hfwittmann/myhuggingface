...
def get_texts(annotations_json):
    ...
    manydoc_json = list()
    for ix, one_document in tqdm(enumerate(annotations_json), total=len(annotations_json), smoothing=0.1):

        text_filename = one_document["text"].replace("/data", "data")
        myid = text_filename.split("/")[-1].split("-")[0]

        mypaths = glob(f"data/upload/1/*{myid}*", recursive=True)  # TODO : auslagern in parameters
        ...
        if "label" in one_document.keys():
            ...
            labels = align_tokens_and_annotations_bilou(tokens, charix_to_tokenix_txt, annotations)
...
def preprocess_and_split(annotations_and_texts_json):
    ...
    manydoc_json_filtered_bytype = [
        one_doc for one_doc in annotations_and_texts_json if "U-wikipedia" in one_doc["ner_tags"]
    ]
    # filter length <= 512
    ...
    replacement = {"O": 0, "B-DateOfBirth": 1, "I-DateOfBirth": 2, "U-DateOfBirth": 3}

    type_filter = list(replacement.keys())[1:]
    # ['B-DateOfBirth', 'I-DateOfBirth', 'U-DateOfBirth']

    for one_doc in manydoc_json_filtered_bytype:
        one_doc["ner_tags"] = [n if n in type_filter else "O" for n in one_doc["ner_tags"]]
    ...
    resulting_dataset_dict = DatasetDict(
        {
            ...
            "validation": Dataset.from_pandas(pd.DataFrame(train_validate), split="validation"),
        }
    )
    ...
    resulting_dataset_dict = DatasetDict(
        {
            ...
            "validation": Dataset.from_pandas(pd.DataFrame(tmp_list_validation), split="validation"),
        }
    )
...
if __name__ == "__main__":
    ...
    os.chdir("/home/hfwittmann/Sync/mygrive/Colab Notebooks/myhuggingface/token-classification")

    annotations_json_file = "data/00_annotations/project-1-at-2022-07-31-12-01-fecd9d73.json"
    with open(annotations_json_file, "r") as f:
        annotations_json = json.load(f)

    get_texts(annotations_json)
